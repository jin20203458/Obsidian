...
<font color="#00b050">8) release 된 메모리 영역을 사용하지 말아야 한다.</font>

<font color="#00b050">9) copy operator를 통해서, 복사되지 않는 멤버 변수가 존재하지 말아야 한다.</font>

<font color="#00b050">10) C++에서 동적 메모리 할당은 반드시 `new` 연산자를 사용해야 한다.</font>

<font color="#00b050">11) 순수 가상 함수는 반드시 `= 0` 으로 초기화해야 한다.</font>

<font color="#00b050">12) 순수 함수는 반드시 `virtual`로 선언되어야 한다.</font>

<font color="#00b050">13) virtual base 클래스 포인터를 derived 클래스 포인터로 변환할 때에는 반드시 `dynamic_cast`를 사용해야 한다.</font>

<font color="#00b050">14) 생성자/소멸자 내에서는 가상함수를 반드시 식별자(Qualifier)를 붙여서 호출해야 한다.</font>

<font color="#00b050">15) 생성자 또는 소멸자에서는 `dynamic_cast`를 사용해서는 안 된다.</font>
...
CFG 에서 싸이클이 있을때 가보지 않았던 CFG 블럭을 병합하는 문제 발생(해결)
