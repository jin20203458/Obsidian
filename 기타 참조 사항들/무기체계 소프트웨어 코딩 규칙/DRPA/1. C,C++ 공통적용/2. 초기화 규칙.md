...
<font color="#00b050">가) char 배열의 초기화시 char 배열의 크기는 초기화 문자열의 크기와 Null을 포함하는 크기로 선언되어야 한다.</font>(tidy)

<font color="#00b050">나) 변수는 사용전 반드시 초기화되어야 한다.</font>(tidy,CSA)

<font color="#00b050">다) 값이 초기화되지 않은 변수의 포인터를 함수의 Read-only(const)로 사용해서는 안된다.</font>(tidy,CSA) (수정필요 , 주석참조)
...
[[UndefinedAssignmentChecker.cpp]] : CSA (정의되지 않는 대입)

| 체크 이름                                           | 설명                         |
| ----------------------------------------------- | -------------------------- |
| `clang-analyzer-core.uninitialized.Assign`      | 대입 시 초기화되지 않은 변수 탐지        |
| `clang-analyzer-core.uninitialized.Branch`      | 조건문에서 초기화되지 않은 변수 사용 탐지    |
| `clang-analyzer-core.uninitialized.UndefReturn` | 초기화되지 않은 값을 반환하는 경우 탐지     |
| `clang-analyzer-core.CallAndMessage`            | 초기화되지 않은 값을 함수 인자로 넘길 때 탐지 |
[[InitBeforeUseCheck]] : Clang-tidy 
### ❗ 잘못된 예 (위반 사례)
