...
| 08 | Common_Style | 아 | `NoGotoCheck` | ✅ `missingReturn` |
...
| 56 | CPP_Specific_1 | 5 | `UnknownChecker` | ✅ `constVariableReference` |
| 58 | CPP_Specific_1 | 7 | `MainUnhandledThrowCheck` | ✅ `throwInEntryPoint` |
...
| 31 | Common_Conv | 사 | 포인터 Cast의 결과로 인한 이전 포인터의 CONST 특성의 상실을 유의해야한다. | `PointerCvQualifierDropCheck` | ⚠️ 결함 진단 실패 (단순 파생 경고만 발생: `cstyleCast`, `constVariablePointer`, `unreadVariable`) |
| 32 | Common_Conv | 아 | 포인터 Cast의 결과로 인한 이전 포인터의 volatile 특성 상실을 유의해야 한다. | `PointerCvQualifierDropCheck` | ⚠️ 결함 진단 실패 (단순 파생 경고만 발생: `cstyleCast`, `constVariablePointer`, `unreadVariable`) |
...
| 49 | C_Specific | 3 | malloc에서 사용하는 인자값은 assign될 변수 type의 정수배이어야 한다. | `MallocSizeMultipleCheck` | ⚠️ 결함 진단 실패 (단순 파생 경고만 발생: `redundantAssignment`, `cstyleCast`, `memleak`, `constVariablePointer`, `unusedAllocatedMemory`, `unreadVariable`) |
...
| 63 | CPP_Specific_2 | 12 | 순수 함수는 반드시 `virtual`로 선언되어야 한다. | `VirtualPureCheck` | ⚠️ 결함 진단 실패 (단순 파생 경고만 발생: `functionStatic`, `duplInheritedMember`) |
| 64 | CPP_Specific_2 | 13 | virtual base 클래스 포인터를 derived 클래스 포인터로 변환할 때에는 반드시 `dynamic_cast`를 사용해야 한다. | `VirtualBaseCastCheck` | ⚠️ 결함 진단 실패 (단순 파생 경고만 발생: `redundantAssignment`, `uselessAssignmentPtrArg`, `unreadVariable`, `missingOverride`) |
