...
## 2. CTU (Cross-Translation Unit) 분석 한계
단일 파일(AST) 단위 분석만 진행하는 현재 환경의 구조적 한계입니다. 여러 소스 파일(Object)에 걸쳐 존재하는 중복이나 불일치는 찾아낼 수 없습니다.
...
> **대응 방안**: 전역 심볼 중복 및 타입 불일치는 링커(Linker) 단계에서 에러로 잡히거나, 전체 프로젝트 단위의 CTU (Cross-Translation Unit) 분석 아키텍처를 연동해야만 탐지할 수 있습니다.
