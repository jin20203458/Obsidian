# ArqaStatic (Clang-Tidy) 검증 도구 분석 및 Cppcheck 비교 보고서

본 보고서는 LLVM/Clang 인프라 기반의 국산 정적 분석 도구인 **ArqaStatic (clang-tidy 엔진)**을 활용하여 66종의 DAPA 무기체계 소프트웨어 코딩 규칙 검증 테스트 슈트를 분석한 결과와, 기존 **Cppcheck**의 진단 성능을 비교 분석한 자료입니다.

---

## 1. 📊 종합 진단 성능 비교 요약 (ArqaStatic vs Cppcheck)

| 도구 (Tool) | 분석 대상 규칙 | 제외 (Placeholder) | 검출 성공 (TP) | 미탐 (FN) | **최종 검출률 (%)** |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **ArqaStatic (Clang-Tidy)** | 66종 | 1종 | **53종** | **12종** | **81.5%** |
| **Cppcheck** | 66종 | 1종 | **29종** | **36종** | **44.6%** |

> 💡 **진단 분석**: ArqaStatic은 AST(추상 구문 트리) 기반의 분석 엔진과 매핑 체커를 통해 **81.5%**의 높은 검출률을 달성하였습니다. 이는 기존 Cppcheck의 검출 한계(44.6%)를 완벽하게 보완하며, Cppcheck가 탐지하지 못했던 **코딩 스타일 규약, 타입 변환 제한, 전처리 규칙** 등에서 압도적인 진단율을 증명합니다.

---

## 2. 📈 룰셋 분류별(Category) 검출 성능 비교

| 분류 카테고리 (Category) | 분석 대상 수 | ArqaStatic 검출 수 | Cppcheck 검출 수 | ArqaStatic 탐지율 | Cppcheck 탐지율 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| 공통 적용 (코딩 스타일) | 11 | **10** | **2** | **90.9%** | 18.2% |
| 공통 적용 (초기화) | 3 | **2** | **2** | **66.7%** | 66.7% |
| 공통 적용 (식별자) | 4 | **2** | **1** | **50.0%** | 25.0% |
| 공통 적용 (조건식) | 5 | **3** | **2** | **60.0%** | 40.0% |
| 공통 적용 (타입 변환) | 8 | **7** | **2** | **87.5%** | 25.0% |
| 공통 적용 (포인터 및 배열) | 5 | **5** | **4** | **100.0%** | 80.0% |
| 공통 적용 (연산식) | 9 | **8** | **5** | **88.9%** | 55.6% |
| C 전용 코딩 규칙 | 5 | **4** | **1** | **80.0%** | 20.0% |
| C++ 전용 코딩 규칙 | 15 | **12** | **10** | **80.0%** | 66.7% |

---

## 3. 🎯 ArqaStatic (Clang-Tidy) 개별 규칙 상세 진단 로그
ArqaStatic이 소스코드에서 실제 진단한 개별 규칙과 검출 메시지 매핑 테이블입니다.
...
| 06 | Common_Style | 외부 함수 사용시 이를 명시하고 사용해야 한다. | **❌ 미탐** | 미검출 (대상 체커: `ExternFunctionDeclarationCheck`) |
...
| 13 | Common_Init | char 배열의 초기화시 char 배열의 크기는 초기화 문자열의 크기와 Null을 포함하는 크기로 선언되어야 한다. | **❌ 미탐** | 미검출 (대상 체커: `CharArrayStringSizeCheck`) |
...
| 16 | Common_Ident | external과 internal linkage의 특성을 동시에 가질 수 없다. | **❌ 미탐** | 미검출 (대상 체커: `UnknownChecker`) |
...
| 22 | Common_Cond | switch의 case의 조건을 만족할 수 없는 Label을 사용하지 않는다. | **❌ 미탐** | 미검출 (대상 체커: `UnknownChecker`) |
| 23 | Common_Cond | Switch 구문에서 Expression을 논리적 연산으로 사용하지 말아야 한다. | **✅ 검출** | `ast-switch-style`: switch문에는 반드시 default label이 포함되어야 합니다. |
| 24 | Common_Cond | 수행되지 않는 소스코드를 작성하지 말아야 한다. | **❌ 미탐** | 미검출 (대상 체커: `UnreachableCodeCheck`) |
...
| 42 | Common_Expr | Underlying type이 Unsigned인 데이터에 '-'를 사용하여 결과를 assign하지 말아야 한다. | **❌ 미탐** | 미검출 (대상 체커: `UnsignedMinusAssignmentCheck`) |
...
| 48 | C_Specific | `#include` 구문에서 표준에 맞지 않는 Character set을 사용하지 않아야 한다. | **❌ 미탐** | 미검출 (대상 체커: `IncludeCharsetCheck`) |
