...
최고의 품질과 안정성을 지닌 프롬프트를 효율적으로 뽑아내기 위해, 기존에 작성된 두 가지 핵심 가이드라인 문서(`AI_Prompt_Engineering_Guidelines.md` 및 `AI_Prompt_AntiPatterns_Guidelines.md`)를 결합한 **"3단계 프롬프트 조립법 (Build - Audit - Test)"**을 따를 것을 권장합니다.

---

## 🏗️ 1단계: 마인드셋 세팅 및 뼈대 잡기 (Engineering 주도)

처음 백지상태에서 프롬프트를 기획하고 작성할 때는 **`AI_Prompt_Engineering_Guidelines.md`**를 설계도처럼 활용합니다.
...
초안이 작성되면 완성이라 생각하지 말고, **`AI_Prompt_AntiPatterns_Guidelines.md`**를 안전 점검표 삼아 프롬프트를 깐깐하게 검수합니다.
