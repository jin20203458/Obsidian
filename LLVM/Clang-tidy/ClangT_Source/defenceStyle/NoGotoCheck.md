...
void NoGotoCheck::registerMatchers(MatchFinder *Finder) {
        Finder->addMatcher(
        gotoStmt(isExpansionInMainFile()).bind("goto"), 
        this);
}

void NoGotoCheck::check(const MatchFinder::MatchResult &Result) {
    const auto *gotoStatement = Result.Nodes.getNodeAs<GotoStmt>("goto");
    if (!gotoStatement) return;
  
    diag(gotoStatement->getGotoLoc(), u8"goto 문은 사용하지 마십시오.");
}
