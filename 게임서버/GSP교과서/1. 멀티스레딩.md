...
**여기서부턴 제 생각입니다.**
CriticalSection 객체는 하나의 프로세스 내 스레드를 빠르게 처리하기 위해서 스핀락을 도입한? 뮤텍스 라고 생각됩니다. 
...
### **Critical Section(임계영역) 객체**

**Critical Section(임계영역) 객체**는 Windows API에서 제공하는 **동기화 메커니즘**으로, 단일 프로세스 내에서 **스레드 간의 상호 배제(Mutual Exclusion)**를 보장합니다. 임계영역 객체는 **공유 자원**에 동시에 여러 스레드가 접근하지 못하도록 보호하며, **Mutex(뮤텍스)**와 비슷한 기능을 하지만, **단일 프로세스 내에서만 사용**될 수 있습니다.
